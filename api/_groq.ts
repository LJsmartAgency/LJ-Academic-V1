...
export async function groqChat(
  ...
  for (const model of list) {
    const r = await fetch(`${GROQ_BASE}/chat/completions`, {
      ...
      body: JSON.stringify({ model, messages, temperature: opts.temperature ?? 0.7, max_tokens: Math.min(8000, opts.max_tokens ?? 2000) }),
    });
    if (r.ok) {
      const d = await r.json();
    ...
    if (r.status === 429) return { ok: false, status: 429, error: "Limite de pedidos atingido. Tente novamente em instantes." };
    ...
    return { ok: false, status: 500, error: `Groq ${r.status}: ${errText.slice(0, 300)}` };
  }
